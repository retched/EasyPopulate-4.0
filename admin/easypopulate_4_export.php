...
while ($row = ($ep_uses_mysqli ? mysqli_fetch_array($result) : mysql_fetch_array($result))) {
    ...
    if ($ep_dltype == 'attrib_basic') { // special case 'attrib_basic'
        ...
        if (($ep_dltype == 'SBAStock')) {
            ...
            ' 
				FROM '.TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS.' as ovpo,'.TABLE_PRODUCTS_OPTIONS_VALUES.' as v,'.TABLE_PRODUCTS.' as p,'.TABLE_PRODUCTS_OPTIONS.' as o,'.TABLE_PRODUCTS_ATTRIBUTES.' a LEFT JOIN '.TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK.' pwas ON pwas.stock_attributes = a.products_attributes_id
				WHERE
		...
		s.customid            as v_customid ' : ' ').'FROM '.TABLE_PRODUCTS.' as p,'.TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK.' as s
				WHERE 
                ...
                while ($rowSBA = ($ep_uses_mysqli ? mysqli_fetch_assoc($resultSBA) : mysql_fetch_assoc($resultSBA))) {
                    ...
                    foreach($trackAttribute as $currentAttrib) {
                        $sqlSBAAttributes = 'SELECT
							...
							FROM '.TABLE_PRODUCTS_OPTIONS.' as o, '.TABLE_PRODUCTS_OPTIONS_VALUES.' as v, '.TABLE_PRODUCTS_ATTRIBUTES.' as a
							WHERE 
