...
require_once(DIR_FS_ADMIN . 'includes/classes/' . /*DIR_WS_CLASSES . */ 'class.EP4CeonURIMappingAdminProductPages.php');
...
//			} // end of special case 'attrib_basic'
	...
	if ($ep_dltype == 'full') {
		...
		foreach ($langcode as $key => $lang) {
			...
			$prev_uri_mappings = $row['v_current_uri'];
			$uri_mappings = $row['v_current_uri'];
			$uri_mapping_autogen = '1';
			...
			$current_category_id = $row['v_category_id'];
			
			 // if information is posted then go through data and collect/interprelate
			 $Mappings = $ceon_uri_mapping_admin->productPreviewProcessSubmission($current_category_id, $prev_uri_mappings, $uri_mappings, $uri_mapping_autogen, $products_name, $products_model, $master_category, $pID);
			//print_r($Mappings);
			//$row['v_uri'] = $filelayout['v_uri'];
