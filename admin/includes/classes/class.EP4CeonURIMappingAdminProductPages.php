 ...
 * @version     $Id: class.EP4CeonURIMappingAdminProductPages.php 1054 2014-08-04 15:45:15Z mc12345678 EP4.0.24CEON$
 */
 ...
 */
class EP4CeonURIMappingAdminProductPages extends CeonURIMappingAdminProducts
{
	// {{{ properties
	 ...
	 * @return  none
	 */
	function collectInfoHandler($prev_uri_mappings = NULL, $uri_mappings = NULL, $uri_mapping_autogen = NULL, $pID = NULL)
	{
		global $db;
		
		// Get any current product URI mappings from the database
		if (isset($pID) && (!isset($prev_uri_mappings) or !isset($uri_mappings) or !isset($uri_mapping_autogen))) {
	 ...
	 * @param   string    $products_name         The name of the product to be translated into the URI.
	 * @param   string    $products_model        The model of the product to be translated into the URI.
	 * @param   integer   $master_category       Needed if a linked product is having its master category changed.
	 * @param   integer   $pID                   Products ID number that is considered optional.
	 * @return  none
	 */
	function productPreviewProcessSubmission($current_category_id, $products_name, $products_model, $master_category = NULL, $pID = NULL)
	{
		global $db;
		...
		// displayed
		//$this->_uri_mapping_autogen = ((is_null($uri_mapping_autogen)) ? true : false);
		
		if ($this->_uri_mapping_autogen) {
			// Is a linked product having its master category changed?
			if (isset($master_category) && $master_category > 0) {
				$master_categories_id = $master_category;
			} else {
			...
			for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
				$product_id = (isset($pID) ? (int) $pID : null);
		...
		for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
			...
			if (!$existing_uri_mapping_result->EOF &&
					...
					(zen_get_handler_from_type(zen_get_products_type((int) $pID)) . '_info') &&
					$existing_uri_mapping_result->fields['associated_db_id'] == $pID &&
	 ...
	 * @param	  array	  $prev_uri_mappings The product's URI mappings before update.
	 * @param	  array	  $uri_mappings   The product's proposed "new" URI mappings
	 * @return  none
	 */
	function updateProductHandler($product_id, $product_type, $prev_uri_mappings, $uri_mappings)
	{
		...
		for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
			...
			$uri_mapping = trim($uri_mappings[$languages[$i]['id']]);
			
			$insert_uri_mapping = false;
			$update_uri_mapping = false;
			
			//The following may have issues depending on if sending a NULL or value(s) of mapping.  Should just be able to use stored data.  If a prev URI exists need to give it ir else over populate the database.
			//This is to identify if the provided data supports an indicated need to update the database.
			if ($uri_mapping != '' && $uri_mapping != NULL) {
			...
			} else if (zen_not_null($prev_uri_mapping) && $uri_mapping == '') {
