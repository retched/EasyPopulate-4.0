...
class EP4CeonURIMappingAdminCategoryPages extends CeonURIMappingAdminCategories
{
	 ...
	 * @access  public
	 */
	function __construct() // EP4CeonURIMappingAdminCategoryPages
	{
	...
	function insertUpdateHandler($category_id, $current_category_id, $prev_uri_mappings, $uri_mappings, $categories_name, $uri_mapping_autogen = NULL)
	{
		...
		$uri_mapping_autogen = (isset($uri_mapping_autogen) ? true : false); //Not functionally used, as it is an all or nothing switch and does not account for individual languages/situations.
		...
		for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
			...
			if ($uri_mapping_autogen) {
				...
				//Following condition does not appear that it can exist because will not be in this loop if the item is not to be autogenerated.  Therefore this logic is intended to prevent autogeneration and would not be activated...; However, there is a goal to autogenerate the information but not to capture it in the database, so the autogeneration should consider that switch as part of the process.
			...
			if ($insert_uri_mapping || $update_uri_mapping) {
				if ($update_uri_mapping && EP4_EXPORT_CAT_ONLY != 1) {
					// Consign previous mapping to the history, so old URI mapping isn't broken
					//Don't want to do this if exporting the information only.
				...
				//Don't want to actually do this if exporting the information only, but it is possible that the result will not get captured properly if not performed.  Ideally could add it, get result then remove it.
				if (EP4_EXPORT_CAT_ONLY == 1) {
				...
				//Don't want to actually do this if exporting the information only.
