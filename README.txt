...
7) CEON URI support. At this time, Updating of URI rewriting is provided on an export basis only. (After importing new unmapped products, simply export the database and the URIs will be updated/inserted as identified in the configuration settings.)  Support is being developed to "reverse" the process and to populate the applicable database information with uploaded URIs.  The current "concern" or delay is determining the method(s) to provide users to perform the upload(s).  Should a single file for upload include all active (current) and inactive (historical) URIs? Should a file be provided to upload only inactive URIs that if an inactive is currently active to also have an admin config switch on how to address (ignore or make active inactive?), should an individual active file be provided to verify/modify the URIs...  Too many "uncontrolled" options though "flexible" could result in database chaos and a broken site.  Ideally controls would be in place such that certain EP4 actions would not be possible.

So, what to do?  With CEON URI Mapping installed, EP4 with URI support can be added to the system without affecting the existing CEON URI Mapping installation, and for that matter it adds functionality to your existing EP4 installation if it is already installed or brings you into the world of EP4 and rapid site updating if you have not used it.  Upon installation of this version of EP4 the settings will automatically support autocreating information that does not currently exist in the database as the applicable export is performed.  This will not overwrite any existing URIs that may have been personalized. Other options include an export only of the uri rewriting results and various options for overwriting all existing URIs: (overwrite all uris or possibly overwrite only existing uris, say if there was a change of some type that necessitated existing URIs to be rewritten).  The rewrite is with what would be a "default" URI when autogenerated (for example if an initial product were added to the database, and the URI had been manually entered to something other than what would be considered the auto-generated URI, then when "all" URIs are overwritten, that product will have a current URI of the "standard" rewritten URI, and still have a historical mapping that would work as well or if incorrectly stated, this will follow the process of changing a URI as if in the product page the new URI were typed and the product saved.)  The settings for the autogeneration format are in the CEON URI mapping settings and are not controlled by this module.  General instructions of usage of thee added capability of this modification can be found below beginning with the section 'CEON URI Basic Usage Instruction'.  See existing instruction for the CEON URI plugin for settings, options, and operation of that plugin.
...
CEON URI Basic Usage Instruction:

1. Settings to populate the database with fresh/new rewritten URIs for all products:
(NOTE: A combination of Yes and Mixed to the pair of choices below is logically equivalent and is expected to work the same as Yes and Yes.)
  a. Products:
AutoCreate URL For CEON When URL Doesn’t Exist (Yes as the answer – 1 shows in the configuration window)
AutoCreate URL For CEON – All Products (Yes as the answer – 1 shows in the configuration window)
  b. Categories:
AutoCreate Category URL For CEON When URL Doesn’t Exist (Yes as the answer – 1 shows in the configuration window)
AutoCreate Category URL For CEON – All Categories (Yes as the answer – 1 shows in the configuration window)
  c. EZ-Pages:
AutoCreate EZ-Page URL For CEON When URL Doesn’t Exist (Yes as the answer – 1 shows in the configuration window)
AutoCreate EZ-Page URL For CEON – All EZ-Pages (Yes as the answer – 1 shows in the configuration window)

2. Settings to populate the database with fresh/new rewritten URIs for new items/entries (not yet rewritten neither automatically nor manually)
  a. Products:
AutoCreate URL For CEON When URL Doesn’t Exist (Yes as the answer – 1 shows in the configuration window)
AutoCreate URL For CEON – All Products (No as the answer –0 shows in the configuration window)
  b. Categories:
AutoCreate Category URL For CEON When URL Doesn’t Exist (Yes as the answer – 1 shows in the configuration window)
AutoCreate Category URL For CEON – All Categories (No as the answer – 0 shows in the configuration window)
  c. EZ-Pages:
AutoCreate EZ-Page URL For CEON When URL Doesn’t Exist (Yes as the answer – 1 shows in the configuration window)
AutoCreate EZ-Page URL For CEON – All EZ-Pages (No as the answer – 0 shows in the configuration window)

3. Settings to populate the database with fresh/new rewritten URIs for existing items/entries only (does not modify/populate the database for new items or items without rewritten URIs)
  a. Products:
AutoCreate URL For CEON When URL Doesn’t Exist (No as the answer – 0 shows in the configuration window)
AutoCreate URL For CEON – All Products (Mixed as the answer –2 shows in the configuration window)
  b. Categories:
AutoCreate Category URL For CEON When URL Doesn’t Exist (No as the answer – 0 shows in the configuration window)
AutoCreate Category URL For CEON – All Categories (Mixed as the answer – 2 shows in the configuration window)
  c. EZ-Pages:
AutoCreate EZ-Page URL For CEON When URL Doesn’t Exist (No as the answer – 0 shows in the configuration window)
AutoCreate EZ-Page URL For CEON – All EZ-Pages (Mixed as the answer – 2 shows in the configuration window)

4. Settings to see what the results would be for any of the above without actually populating the database/changing existing condition:
  a. Products:
Selections made to see the results of the above, then changing/verifying ‘Export URL Information From CEON – All Products’ is set to Yes (1 displays in the configuration window).
  b. Categories:
Selections made to see the results of the above, then changing/verifying ‘Export URL Information From CEON – All Categories’ is set to Yes (1 displays in the configuration window).
  c. EZ-Pages:
Selections made to see the results of the above (Sections 1, 2, or 3), then changing/verifying ‘Export URL Information From CEON – All EZ-Pages’ is set to Yes (1 displays in the configuration window).

5. Settings to see the current active URI without actually populating the database/changing existing condition:
  a. Products:
AutoCreate URL For CEON When URL Doesn’t Exist (No as the answer – 0 shows in the configuration window)
AutoCreate URL For CEON – All Products (No as the answer –0 shows in the configuration window)
  b. Categories:
...
To export/populate URIs for products from the tools->Easy Populate 4 menu: select   Complete Products  (with Metatags and CEON export)
To export/populate URIs for categories from the tools->Easy Populate 4 menu: select   Categories Only  (with Metatags and CEON export)
To export/populate URIs for EZ-Pages from the tools->Easy Populate 4 menu: select   EZ Pages CEON Data (Export)
